...
void    *ft_philo_routine(void *data)
{
    ...
    while (1)
    {            
        if ((philo->data->dead_flag == 0 && philo->data->start_flag == 1) && (philo->times_eat < philo->data->eat_nbr || philo->data->eat_nbr == -1))
...
void    ft_start_program(t_data *data)
{
    ...
    while (++i <= (data)->p_nbr)
    {
        if (pthread_create(&(c_philo)->thread, NULL, &ft_philo_routine, (void *)c_philo) != 0)
